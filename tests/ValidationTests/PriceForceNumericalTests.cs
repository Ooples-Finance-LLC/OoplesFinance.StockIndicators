...
public sealed class PriceForceNumericalTests
{
    ...
    [Fact]
    public void FusedForceProductsKeepSelectedPricesAndRecoverThroughAverages()
    {
        ...
        foreach (var indicator in new IBuiltInIndicator[] { new ForceIndex(period), new ElderForceIndex(period), new ForceIndex(period, new Sma()), new ForceIndex(period, new Wma()), new ForceIndex(period, new Smma()) })
        {
            var bars = BarsOf(prices).Select(b => new Bar(b.Time, b.Open, b.High, b.Low, b.Close, scale)).ToArray();
            var expected = BuiltInFormulaReferences.ForceOutputs(bars, indicator)["Fi"];
            var kind = indicator.CreateOptions() is ForceIndexSpecOptions options ? options.MaType : MovingAvgType.ExponentialMovingAverage;
            Assert.Equal(expected, Data(bars).CalculateForceIndex(kind, period).CustomValuesList);
            ...
            Assert.Equal(expected, selected.CalculateForceIndex(kind, period).CustomValuesList);
            var core = new double[bars.Length]; var volumes = bars.Select(b => b.Volume).ToArray();
            OoplesFinance.StockIndicators.Core.VolumeCore.ForceIndex(prices, volumes, core, period, kind); Assert.Equal(expected, core);
            if (kind == MovingAvgType.ExponentialMovingAverage)
            { OoplesFinance.StockIndicators.Core.OscillatorCore.ElderForceIndex(prices, volumes, core, period); Assert.Equal(expected, core); }
            using var state = new ForceIndexState(kind, period);
        ...
        var recover = BarsOf(new[] { 0d, 2, 0 }).Select(b => new Bar(b.Time, b.Open, b.High, b.Low, b.Close, double.MaxValue)).ToArray();
        var expectedRecovery = BuiltInFormulaReferences.ForceOutputs(recover, new ForceIndex(2, new Sma()))["Fi"];
        Assert.Equal(new[] { 0d, double.MaxValue, 0 }, expectedRecovery);
        Assert.Equal(expectedRecovery, Data(recover).CalculateForceIndex(MovingAvgType.SimpleMovingAverage, 2).CustomValuesList);
    }
    ...
    [Fact]
    public async Task CustomerAverageReceivesRoundedForceProducts()
    {
        var bars = BarsOf(new[] { 2d, 5, 3, 8, 1 }).Select((b, i) => new Bar(b.Time, b.Open, b.High, b.Low, b.Close, i + 1)).ToArray();
        var indicator = new ForceIndex(3, new ConstantAverage(42)); var builtIn = (IBuiltInIndicator)indicator;
        var expected = BuiltInFormulaReferences.ForceOutputs(bars, builtIn, bars.Select(_ => 42d).ToArray())["Fi"];
        using var run = await new StockIndicatorBuilder().ConfigureSource(Bars.From(bars)).ConfigureIndicators(indicator).BuildAsync();
        Assert.Equal(expected, run[indicator].ToArray());
        var rawForces = bars.Select((b, i) => i == 0 ? 0 : (b.Close - bars[i - 1].Close) * b.Volume).ToArray();
        using var armed = ComponentAverage.Arm((input, _) => { Assert.Equal(rawForces, input); return input.Select(_ => 42d).ToArray(); });
        using var context = new ComputeContext();
    ...
    public void InvalidFieldsNeverAdvanceAnyState()
    {
        ...
        {
            using var state = new ForceIndexState(MovingAvgType.WeightedMovingAverage, 3); using var control = new ForceIndexState(MovingAvgType.WeightedMovingAverage, 3);
    ...
    internal static readonly HashSet<string> Families = new(StringComparer.Ordinal) { "ForceIndex", "ElderForceIndex" };
    ...
    public void AllRoutesMatchIndependentFormulasIncludingPreviewAndReset(IndicatorValidationCase testCase, string route) =>
        new OrdinalFamilyNumericalTests().CheckRoutes(testCase, route, bars => BuiltInFormulaReferences.ForceOutputs(bars, (IBuiltInIndicator)testCase.Factory()), IndicatorErrorBudget.Exact);
