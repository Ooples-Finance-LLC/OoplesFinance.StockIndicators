...
public sealed class RecursiveTrendNumericalTests
{
    ...
    public void CascadesPreserveStagesCancellationPreviewAndReset()
    {
        ...
        {
            var bars = BarsOf(prices); IBuiltInIndicator indicator = new RecursiveMovingTrendAverage(length);
            var expected = BuiltInFormulaReferences.RecursiveTrendOutputs(bars, length).Values.Single();
            var batch = Data(bars).CalculateRecursiveMovingTrendAverage(length: length);
            Assert.Equal(expected, batch.CustomValuesList);
            var core = new double[bars.Length];
            OoplesFinance.StockIndicators.Core.MovingAverageCore.RecursiveMovingTrendAverage(prices, core, length);
            ...
            using var actual = IndicatorCompute.ComputeArm(selected, new IndicatorSpec(indicator.BatchName, indicator.CreateOptions(), "Rmta"), context);
            Assert.NotNull(actual); Assert.Equal(expected, actual.Value.ToArray());
            IStreamingIndicatorState state = new RecursiveMovingTrendAverageState(length: length);
    ...
    [Fact]
    public void UnitPeriodRetainsThePublishedMomentumCorrection() =>
        Assert.Equal(new[] { 1d, 3, 6 }, Data(BarsOf(new[] { 1d, 2, 4 })).CalculateRecursiveMovingTrendAverage(1).CustomValuesList);
    ...
    public void InvalidFieldsNeverAdvanceAnyState()
    {
        ...
        {
            IStreamingIndicatorState state = new RecursiveMovingTrendAverageState(length: 3);
            IStreamingIndicatorState control = new RecursiveMovingTrendAverageState(length: 3);
    ...
    public static IEnumerable<object[]> Cases => IndicatorValidationDiscovery.Discover(new[] { typeof(IIndicator).Assembly })
        .Where(c => c.IndicatorType == typeof(RecursiveMovingTrendAverage)).Select(c => new object[] { c });
    public static IEnumerable<object[]> Routes => Cases.SelectMany(c => new[] { "batch", "fast", "arm", "native", "streaming" }.Select(route => new object[] { c[0], route }));
    [Theory, MemberData(nameof(Routes))]
    public void AllRoutesMatchIndependentFormulasIncludingPreviewAndReset(IndicatorValidationCase testCase, string route) =>
        new OrdinalFamilyNumericalTests().CheckRoutes(testCase, route, bars => BuiltInFormulaReferences.RecursiveTrendOutputs(bars, ((RecursiveMovingTrendAverageSpecOptions)((IBuiltInIndicator)testCase.Factory()).CreateOptions()).Length), IndicatorErrorBudget.Exact);
