...
public sealed class StochasticNumericalTests
{
    ...
    public void DiscoveryIncludesEveryPromotedComposition()
        => Assert.Equal(285, Cases.Count(row => ((IndicatorValidationCase)row[0]).Name.StartsWith("stochastic-composition/")));
    ...
    public void DiscoveryIncludesDoubleStochasticCompositions()
        => Assert.Equal(57, Cases.Count(row => ((IndicatorValidationCase)row[0]).Name.StartsWith("double-stochastic-composition/")));
    ...
    public void DiscoveryIncludesDynamicMomentumCompositions()
        => Assert.Equal(57, Cases.Count(row => ((IndicatorValidationCase)row[0]).Name.StartsWith("dynamic-momentum-composition/")));
