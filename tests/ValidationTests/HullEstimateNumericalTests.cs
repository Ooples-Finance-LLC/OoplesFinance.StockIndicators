...
public sealed class HullEstimateNumericalTests
{
    ...
    public void CascadesPreserveStagesCancellationPreviewAndReset()
    {
        ...
        {
            var bars = BarsOf(prices); IBuiltInIndicator indicator = new HullEstimate(length);
            var expected = BuiltInFormulaReferences.HullEstimateOutputs(bars, length).Values.Single();
            var batch = Data(bars).CalculateHullEstimate(length: length);
            Assert.Equal(expected, batch.CustomValuesList);
            var core = new double[bars.Length];
            OoplesFinance.StockIndicators.Core.MovingAverageCore.HullEstimate(prices, core, length);
            Assert.Equal(expected, core);
            Assert.Equal(expected, CalculationsHelper.GetMovingAverageList(Data(bars), MovingAvgType.HullEstimate, length, prices.ToList()));
            var selected = Data(BarsOf(prices.Select(_ => 42d).ToArray())); selected.CustomValuesList = prices.ToList();
            using var context = new ComputeContext();
            using var actual = IndicatorCompute.ComputeArm(selected, new IndicatorSpec(indicator.BatchName, indicator.CreateOptions(), "He"), context);
            Assert.NotNull(actual); Assert.Equal(expected, actual.Value.ToArray());
            IStreamingIndicatorState state = new HullEstimateState(length: length);
    ...
    [Fact]
    public void CustomerAveragesReceiveTheSameSourceIndependently()
    {
        ...
        var callbacks = new Func<IReadOnlyList<double>, int, IReadOnlyList<double>>[] {
            ...
            (values, period) => { Assert.Equal(3, period); Assert.Equal(bars.Select(b => b.Close), values); return values.Select(_ => 9d).ToArray(); } };
        using var armed = ComponentAverage.Arm(callbacks); using var context = new ComputeContext();
        using var actual = IndicatorCompute.ComputeHullEstimateFast(Data(bars), context, 5);
    ...
    public void InvalidFieldsNeverAdvanceAnyState()
    {
        ...
        {
            IStreamingIndicatorState state = new HullEstimateState(length: 3);
            IStreamingIndicatorState control = new HullEstimateState(length: 3);
    ...
    public static IEnumerable<object[]> Cases => IndicatorValidationDiscovery.Discover(new[] { typeof(IIndicator).Assembly })
        .Where(c => c.IndicatorType == typeof(HullEstimate)).Select(c => new object[] { c });
    public static IEnumerable<object[]> Routes => Cases.SelectMany(c => new[] { "batch", "fast", "arm", "native", "streaming" }.Select(route => new object[] { c[0], route }));
    [Theory, MemberData(nameof(Routes))]
    public void AllRoutesMatchIndependentFormulasIncludingPreviewAndReset(IndicatorValidationCase testCase, string route) =>
        new OrdinalFamilyNumericalTests().CheckRoutes(testCase, route, bars => BuiltInFormulaReferences.HullEstimateOutputs(bars, ((HullEstimateSpecOptions)((IBuiltInIndicator)testCase.Factory()).CreateOptions()).Length), IndicatorErrorBudget.Exact);
