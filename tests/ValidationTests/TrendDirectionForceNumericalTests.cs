...
public sealed class TrendDirectionForceNumericalTests
{
    ...
    [Fact]
    public void FourthDegreeForceKeepsTinyAndExtremeSignalsAndEvictsMaxima()
    {
        ...
        foreach (var lookback in new[] { 1, 4 })
        foreach (var indicator in new IBuiltInIndicator[] { new TrendDirectionForceIndex(period, lookback), new TrendDirectionForceIndex(period, lookback, new Sma()),
            new TrendDirectionForceIndex(period, lookback, new Wma()), new TrendDirectionForceIndex(period, lookback, new Smma()) })
        {
            var bars = BarsOf(prices); var expected = BuiltInFormulaReferences.TrendForceOutputs(bars, indicator)["Tdfi"];
        ...
        var pulse = BarsOf(Enumerable.Repeat(100d, 128).Concat(new[] { 200d, 100, 100 }).ToArray());
        var line = BuiltInFormulaReferences.TrendForceOutputs(pulse, new TrendDirectionForceIndex(10, 1))["Tdfi"];
        Assert.Equal(new[] { 1d, -1, 0 }, line.Skip(128).ToArray());
    }

    [Fact]
    public void CustomerBothAveragesAreUsedInOrder()
    {
        var bars = BarsOf(new[] { 2d, 5, 3, 8, 1 });
        using var armed = ComponentAverage.Arm(new[] { 6d, 2 }.Select(v =>
            (Func<IReadOnlyList<double>, int, IReadOnlyList<double>>)((input, _) => input.Select(_ => v).ToArray())).ToArray());
        using var context = new ComputeContext(); var indicator = (IBuiltInIndicator)new TrendDirectionForceIndex(3, 2);
        ...
        Assert.Equal(new[] { 1d, 0, 0, 0, 0 }, raw.Value.ToArray());
    }
    ...
    public void InvalidFieldsNeverAdvanceAnyState()
    {
        ...
        {
            IStreamingIndicatorState Create() => new TrendDirectionForceIndexState(MovingAvgType.WeightedMovingAverage, 3, 7);
    ...
    internal static readonly HashSet<string> Families = new(StringComparer.Ordinal) { "TrendDirectionForceIndex" };
    ...
    public void AllRoutesMatchIndependentFormulasIncludingPreviewAndReset(IndicatorValidationCase testCase, string route) =>
        new OrdinalFamilyNumericalTests().CheckRoutes(testCase, route, bars => BuiltInFormulaReferences.TrendForceOutputs(bars, (IBuiltInIndicator)testCase.Factory()), IndicatorErrorBudget.Exact);
