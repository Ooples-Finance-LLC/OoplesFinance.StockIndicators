...
public sealed class EfficientPriceNumericalTests
{
    ...
    public void EfficiencyOutputsPreservePreviewResetAndExtendedRange()
    {
        ...
        {
            var bars = BarsOf(prices); IBuiltInIndicator indicator = new EfficientPrice(length);
            var expected = BuiltInFormulaReferences.EfficiencyDerivedOutputs(bars, indicator)["Ep"];
            Assert.Equal(expected, Data(bars).CalculateEfficientPrice(length).CustomValuesList);
            using var context = new ComputeContext(); using var raw = IndicatorCompute.ComputeEfficientPrice(Data(bars), context, length); Assert.Equal(expected, raw.Span.ToArray());
            var state = new EfficientPriceState(length);
    ...
    public void RawAndBatchRespectSelectedInputBeforeCloses()
    {
        ...
        var expected = BuiltInFormulaReferences.EfficiencyDerivedOutputs(BarsOf(selected), new EfficientPrice(3))["Ep"];
        var data = Data(original); data.SetCustomValues(selected.ToList());
        using var context = new ComputeContext(); using var raw = IndicatorCompute.ComputeEfficientPrice(data, context, 3);
        Assert.Equal(expected, raw.Span.ToArray());
        Assert.Equal(expected, data.CalculateEfficientPrice(3).CustomValuesList);
    }
    ...
    public void InvalidFieldsNeverAdvanceAnyState()
    {
        ...
        {
            IStreamingIndicatorState state = new EfficientPriceState(3);
            IStreamingIndicatorState control = new EfficientPriceState(3);
    ...
    public static IEnumerable<object[]> Cases => IndicatorValidationDiscovery.Discover(new[] { typeof(IIndicator).Assembly })
        .Where(c => c.IndicatorType == typeof(EfficientPrice)).Select(c => new object[] { c });
