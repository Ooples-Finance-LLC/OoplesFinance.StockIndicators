...
public sealed class HullNumericalTests
{
    ...
    {
        foreach (var length in new[] { 1, 2, 3, 5, 7, 1063, 282500 })
        ...
        {
            var bars = BarsOf(prices); IBuiltInIndicator indicator = new HullMovingAverage(length);
            var expected = BuiltInFormulaReferences.HullOutputs(bars, indicator).Values.Single();
            var batch = Data(bars).CalculateHullMovingAverage(length: length);
            Assert.Equal(expected, batch.CustomValuesList);
            var core = new double[bars.Length];
            OoplesFinance.StockIndicators.Core.MovingAverageCore.HullMovingAverage(prices, core, length);
            Assert.Equal(expected, core);
            Assert.Equal(expected, CalculationsHelper.GetMovingAverageList(Data(bars), MovingAvgType.HullMovingAverage, length, prices.ToList()));
            var selected = Data(BarsOf(prices.Select(_ => 42d).ToArray())); selected.CustomValuesList = prices.ToList();
            using var context = new ComputeContext();
            using var actual = IndicatorCompute.ComputeArm(selected, new IndicatorSpec(indicator.BatchName, indicator.CreateOptions(), "Hma"), context);
            Assert.NotNull(actual); Assert.Equal(expected, actual.Value.ToArray());
            IStreamingIndicatorState state = new HullMovingAverageState(length: length);
    ...
    public void CustomerStagesReceiveOriginalThenAdjustedInputsInOrder()
    {
        ...
        var callbacks = new Func<IReadOnlyList<double>, int, IReadOnlyList<double>>[] {
            ...
            (values, period) => { Assert.Equal(2, period); Assert.All(values, value => Assert.Equal(13, value)); return values.Select(_ => 7d).ToArray(); } };
        foreach (var batch in new[] { false, true })
        {
            using var armed = ComponentAverage.Arm(callbacks); using var context = new ComputeContext();
            if (batch) Assert.All(Data(bars).CalculateHullMovingAverage(length: 5).CustomValuesList, value => Assert.Equal(7, value));
            else { using var actual = IndicatorCompute.ComputeHullMovingAverageFast(Data(bars), context, 5); Assert.All(actual.Span.ToArray(), value => Assert.Equal(7, value)); }
    ...
    public void InvalidFieldsNeverAdvanceAnyState()
    {
        ...
        {
            IStreamingIndicatorState state = new HullMovingAverageState(length: 3);
            IStreamingIndicatorState control = new HullMovingAverageState(length: 3);
    ...
    public static IEnumerable<object[]> Cases => IndicatorValidationDiscovery.Discover(new[] { typeof(IIndicator).Assembly })
        .Where(c => c.IndicatorType == typeof(HullMovingAverage) || c.IndicatorType == typeof(Hma)).Select(c => new object[] { c });
    public static IEnumerable<object[]> Routes => Cases.SelectMany(c => new[] { "batch", "fast", "arm", "native", "streaming" }.Select(route => new object[] { c[0], route }));
    [Theory, MemberData(nameof(Routes))]
    public void AllRoutesMatchIndependentFormulasIncludingPreviewAndReset(IndicatorValidationCase testCase, string route) =>
        new OrdinalFamilyNumericalTests().CheckRoutes(testCase, route, bars => BuiltInFormulaReferences.HullOutputs(bars, (IBuiltInIndicator)testCase.Factory()), IndicatorErrorBudget.Exact);
