...
public sealed class FastStochasticNumericalTests
{
    public static IEnumerable<object[]> Cases => IndicatorValidationDiscovery.Discover(new[] { typeof(IIndicator).Assembly })
        .Where(c => c.IndicatorType == typeof(StochasticFastOscillator)).Select(c => new object[] { c });

    [Fact]
    public void DiscoveryIncludesEveryPromotedComposition()
        => Assert.Equal(57, Cases.Count(row => ((IndicatorValidationCase)row[0]).Name.StartsWith("stochastic-fast-composition/")));
    ...
    [InlineData(3, 1, 3)]
    [InlineData(14, 3, 1)]
    public async Task AllStagesMatchTheIndependentReferenceAcrossRoutes(int length, int first, int last)
    {
        ...
        foreach (var (average, kind) in averages)
        {
            ...
            var indicator = new StochasticFastOscillator(length, first, last, average);
            using var run = await new StockIndicatorBuilder().ConfigureSource(Bars.From(bars)).ConfigureIndicators(indicator).BuildAsync();
            Assert.Equal(expected["FastD"], run[indicator].ToArray());
            Assert.Equal(expected["SlowD"], run[indicator.Signal].ToArray());
            ...
            data.CalculateStochasticFastOscillator(maType, length, first, last);
            Assert.Equal(expected["FastD"], data.OutputValues["Sfo"]);
            Assert.Equal(expected["SlowD"], data.OutputValues["Signal"]);
            ...
            foreach (var state in new[] { StatefulIndicatorFactory.Create(spec), StreamingIndicatorFactory.CreateState(spec) })
            {
                ...
                for (var replay = 0; replay < 2; replay++)
                {
                    ...
                    foreach (var commit in new[] { false, true })
                    {
                        ...
                        Assert.Equal(expected["FastD"][i], result.Outputs!["Sfo"]);
                        Assert.Equal(expected["SlowD"][i], result.Outputs["Signal"]);
                    }
                }
            }
        }
    ...
    public async Task TypedChainingPreservesOhlcAndConfiguredPeriods()
    {
        ...
        var indicator = new StochasticFastOscillator(3, 1, 7, new Sma());
        indicator.Of(source);
        ...
        var expected = BuiltInFormulaReferences.RoundedStochastic(projected, 3, 1, 7, 1);
        Assert.Equal(expected["FastD"], run[indicator].ToArray());
        Assert.Equal(expected["SlowD"], run[indicator.Signal].ToArray());
    }
}
