...
public sealed class HammingCompositionNumericalTests
{
    public static IEnumerable<object[]> Cases => IndicatorValidationDiscovery.Discover(new[] { typeof(IIndicator).Assembly })
        .Where(c => c.Name.StartsWith("hamming-composition/", StringComparison.Ordinal)
            || c.Name.Contains("-composition/", StringComparison.Ordinal) && c.Name.EndsWith("/21", StringComparison.Ordinal))
    ...
    public void NativeCompositionsAndLegacyMatchIndependentStages(int length)
    {
        foreach (var fixture in IndicatorAdversarialCases.Generate(40, 244))
        {
            ...
            var kind = MovingAvgType.EhlersHammingMovingAverage;
            var referenceKind = 21;
