...
public sealed class NormalizedMacdNumericalTests
{
    ...
    public void ExtremeRatioAndFirstBarSeedHaveIndependentExpectedValues()
    {
        ...
        Core.OscillatorCore.NormalizedMacd(prices, actual, 1, 5);
        Assert.Equal(0, actual[0]);
        Assert.InRange(actual[1], -401, -399);
        var bars = prices.Select((v, i) => new Bar(DateTime.UnixEpoch.AddDays(i), v, v, v, v, 1)).ToArray();
        Assert.Equal(BuiltInFormulaReferences.RoundedNormalizedMacd(bars, new NormalizedMacdSpecOptions(1, 5)), actual);
        Core.OscillatorCore.NormalizedMacd(new[] { 2d, 4 }, actual, 1, 3);
    ...
    public async Task SelectedInputControlsTheBuilder()
    {
        ...
        var indicator = new NormalizedMacd(2, 4).Of(source);
        ...
        Assert.Equal(BuiltInFormulaReferences.RoundedNormalizedMacd(projected, ((IBuiltInIndicator)indicator).CreateOptions()), run[indicator].ToArray());
        ...
        using var buffer = IndicatorCompute.TryComputeFast(data, new IndicatorSpec(Enums.IndicatorName.NormalizedMacd, ((IBuiltInIndicator)indicator).CreateOptions()), context);
        Assert.NotNull(buffer);
        Assert.Equal(BuiltInFormulaReferences.RoundedNormalizedMacd(projected, ((IBuiltInIndicator)indicator).CreateOptions()), buffer.Value.ToArray());
    }

    public static IEnumerable<object[]> Cases => IndicatorValidationDiscovery.Discover(new[] { typeof(IIndicator).Assembly })
        .Where(c => c.IndicatorType == typeof(NormalizedMacd)).Select(c => new object[] { c });
    ...
    public void EveryRouteMatchesIndependentSeededStages(IndicatorValidationCase testCase)
    {
        ...
        {
            var expected = new Dictionary<string, double[]> { ["NormalizedMacd"] = BuiltInFormulaReferences.RoundedNormalizedMacd(fixture.Bars, options) };
            ...
            StockData Data() => new StockData(bars.Select(b => b.Open), bars.Select(b => b.High), bars.Select(b => b.Low),
                bars.Select(b => b.Close), bars.Select(b => b.Volume), bars.Select(b => b.Time));
            var publishedKeys = testCase.Factory().Outputs.Count == 1 ? new[] { builtIn.BatchOutputKey ?? "NormalizedMacd" } : expected.Keys.ToArray();
