...
public sealed class TradeVolumeNumericalTests
{
    ...
    public void ExactContributionsPreserveAliasesSelectionAndState()
    {
        ...
        foreach (var tick in new[] { 0d, .5, -0.5, double.Epsilon, double.MaxValue })
        {
            ...
            IBuiltInIndicator indicator = new TradeVolumeIndex(period, average);
            var bars = BarsOf(prices).Select(b => new Bar(b.Time, b.Open, b.High, b.Low, b.Close, scale)).ToArray();
            var expected = BuiltInFormulaReferences.TradeVolumeOutputs(bars, indicator, minTickValue: tick);
            var key = "Tvi";
            var batch = Data(bars).CalculateTradeVolumeIndex(kind, period, tick);
            Assert.Equal(expected[key], batch.CustomValuesList); Assert.Equal(expected["Signal"], batch.OutputValues["Signal"]);
            using var context = new ComputeContext();
            foreach (var output in tick == .5 ? new[] { key, "Signal" } : Array.Empty<string>())
            {
            ...
            OoplesFinance.StockIndicators.Core.VolumeCore.TradeVolumeIndex(prices, volumes, core, tick); Assert.Equal(expected[key], core);
            using var state = new TradeVolumeIndexState(kind, period, tick);
        ...
        var recovery = BarsOf(new[] { 1d, 2, 1 }).Select(b => new Bar(b.Time, b.Open, b.High, b.Low, b.Close, double.MaxValue)).ToArray();
        var recovered = Data(recovery).CalculateTradeVolumeIndex(MovingAvgType.SimpleMovingAverage, 1);
        Assert.Equal(double.MaxValue, recovered.CustomValuesList[2]);
        Assert.Equal(double.MaxValue, recovered.OutputValues["Signal"][2]);
        var boundary = BarsOf(new[] { .5, 1, .5, 0, -.5, 0, .5 });
        Assert.All(Data(boundary).CalculateTradeVolumeIndex().CustomValuesList, v => Assert.Equal(0, v));
        foreach (var invalid in new[] { double.NaN, double.PositiveInfinity, double.NegativeInfinity })
            Assert.Throws<ArgumentOutOfRangeException>(() => new TradeVolumeIndexState(minTickValue: invalid));
    }
    ...
    public async Task CustomerSignalReceivesCumulativeLineForEveryOutputRequest()
    {
        ...
        foreach (var indicator in new IIndicator[] { new TradeVolumeIndex(3, new ConstantAverage(42)) })
        {
            var builtIn = (IBuiltInIndicator)indicator;
            var expected = BuiltInFormulaReferences.TradeVolumeOutputs(bars, builtIn, bars.Select(_ => 42d).ToArray());
            var key = "Tvi";
    ...
    public void InvalidFieldsNeverAdvanceAnyState()
    {
        ...
        {
            using var state = new TradeVolumeIndexState(); using var control = new TradeVolumeIndexState();
    ...
    internal static readonly HashSet<string> Families = new(StringComparer.Ordinal) { "TradeVolumeIndex" };
    ...
    public void AllRoutesMatchIndependentFormulasIncludingPreviewAndReset(IndicatorValidationCase testCase, string route) =>
        new OrdinalFamilyNumericalTests().CheckRoutes(testCase, route, bars => BuiltInFormulaReferences.TradeVolumeOutputs(bars, (IBuiltInIndicator)testCase.Factory()), IndicatorErrorBudget.Exact);
