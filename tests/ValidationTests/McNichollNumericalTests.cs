...
public sealed class McNichollNumericalTests
{
    ...
    public void CascadesPreserveStagesCancellationPreviewAndReset()
    {
        ...
        {
            var bars = BarsOf(prices); IBuiltInIndicator indicator = new McNichollMovingAverage(length);
            var expected = BuiltInFormulaReferences.McNichollOutputs(bars, indicator).Values.Single();
            var batch = Data(bars).CalculateMcNichollMovingAverage(length: length);
            Assert.Equal(expected, batch.CustomValuesList);
            var core = new double[bars.Length];
            OoplesFinance.StockIndicators.Core.MovingAverageCore.McNichollMovingAverage(prices, core, length);
            Assert.Equal(expected, core);
            using var smoother = MovingAverageSmootherFactory.Create(MovingAvgType.McNichollMovingAverage, length);
            ...
            using var actual = IndicatorCompute.ComputeArm(selected, new IndicatorSpec(indicator.BatchName, indicator.CreateOptions(), "Mnma"), context);
            Assert.NotNull(actual); Assert.Equal(expected, actual.Value.ToArray());
            IStreamingIndicatorState state = new McNichollMovingAverageState(length: length);
    ...
    public void CustomerStagesRemainSequential()
    {
        ...
        using var actual = IndicatorCompute.ComputeMcNichollMovingAverageFast(Data(bars), context, 3);
    ...
    public void InvalidFieldsNeverAdvanceAnyState()
    {
        ...
        {
            IStreamingIndicatorState state = new McNichollMovingAverageState(length: 3);
            IStreamingIndicatorState control = new McNichollMovingAverageState(length: 3);
    ...
    public static IEnumerable<object[]> Cases => IndicatorValidationDiscovery.Discover(new[] { typeof(IIndicator).Assembly })
        .Where(c => c.IndicatorType == typeof(McNichollMovingAverage)).Select(c => new object[] { c });
    public static IEnumerable<object[]> Routes => Cases.SelectMany(c => new[] { "batch", "fast", "arm", "native", "streaming" }.Select(route => new object[] { c[0], route }));
    [Theory, MemberData(nameof(Routes))]
    public void AllRoutesMatchIndependentFormulasIncludingPreviewAndReset(IndicatorValidationCase testCase, string route) =>
        new OrdinalFamilyNumericalTests().CheckRoutes(testCase, route, bars => BuiltInFormulaReferences.McNichollOutputs(bars, (IBuiltInIndicator)testCase.Factory()), IndicatorErrorBudget.Exact);
