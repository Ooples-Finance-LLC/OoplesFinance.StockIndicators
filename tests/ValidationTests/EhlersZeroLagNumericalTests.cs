...
public sealed class EhlersZeroLagNumericalTests
{
    ...
    public void CascadesPreserveStagesCancellationPreviewAndReset()
    {
        ...
        {
            var bars = BarsOf(prices); IBuiltInIndicator indicator = new EhlersZeroLagExponentialMovingAverage(length);
            var expected = BuiltInFormulaReferences.EhlersZeroLagOutputs(bars, indicator).Values.Single();
            var batch = Data(bars).CalculateEhlersZeroLagExponentialMovingAverage(length: length);
            Assert.Equal(expected, batch.CustomValuesList);
            var core = new double[bars.Length];
            OoplesFinance.StockIndicators.Core.MovingAverageCore.EhlersZeroLagExponentialMovingAverage(prices, core, length);
            Assert.Equal(expected, core);
            using var smoother = MovingAverageSmootherFactory.Create(MovingAvgType.EhlersZeroLagExponentialMovingAverage, length);
            ...
            using var actual = IndicatorCompute.ComputeArm(selected, new IndicatorSpec(indicator.BatchName, indicator.CreateOptions(), "Ezlema"), context);
            Assert.NotNull(actual); Assert.Equal(expected, actual.Value.ToArray());
            IStreamingIndicatorState state = new EhlersZeroLagExponentialMovingAverageState(length: length);
    ...
    [Fact]
    public void CustomerAverageReceivesLagAdjustedInputs()
    {
        var bars = BarsOf(new[] { 1d, 2, 4, 8 });
        var callbacks = new Func<IReadOnlyList<double>, int, IReadOnlyList<double>>[] {
            (values, period) => { Assert.Equal(3, period); Assert.Equal(new[] { 1d, 3, 6, 12 }, values); return values.Select(_ => 5d).ToArray(); } };
        using var armed = ComponentAverage.Arm(callbacks); using var context = new ComputeContext();
        using var actual = IndicatorCompute.ComputeEhlersZeroLagEmaFast(Data(bars), context, 3);
        Assert.Equal(1, ComponentAverage.Substitutions); Assert.All(actual.Span.ToArray(), value => Assert.Equal(5, value));
    }
    ...
    public void InvalidFieldsNeverAdvanceAnyState()
    {
        ...
        {
            IStreamingIndicatorState state = new EhlersZeroLagExponentialMovingAverageState(length: 3);
            IStreamingIndicatorState control = new EhlersZeroLagExponentialMovingAverageState(length: 3);
    ...
    public static IEnumerable<object[]> Cases => IndicatorValidationDiscovery.Discover(new[] { typeof(IIndicator).Assembly })
        .Where(c => c.IndicatorType == typeof(EhlersZeroLagExponentialMovingAverage) || c.IndicatorType == typeof(EhlersZeroLagEma)).Select(c => new object[] { c });
    public static IEnumerable<object[]> Routes => Cases.SelectMany(c => new[] { "batch", "fast", "arm", "native", "streaming" }.Select(route => new object[] { c[0], route }));
    [Theory, MemberData(nameof(Routes))]
    public void AllRoutesMatchIndependentFormulasIncludingPreviewAndReset(IndicatorValidationCase testCase, string route) =>
        new OrdinalFamilyNumericalTests().CheckRoutes(testCase, route, bars => BuiltInFormulaReferences.EhlersZeroLagOutputs(bars, (IBuiltInIndicator)testCase.Factory()), IndicatorErrorBudget.Exact);
