...
public sealed class TargetReturnNumericalTests
{
    ...
    [Fact]
    public void ExactReturnsNormalizeBeforeOverflowAndPreservePaddingAndLag()
    {
        ...
        foreach (var indicator in new IBuiltInIndicator[] { new OmegaRatio(period, benchmark), new UpsidePotentialRatio(period, benchmark) })
        {
            var bars = BarsOf(prices); var key = indicator.BatchName == IndicatorName.OmegaRatio ? "Or" : "Upr";
        ...
        Assert.Equal(1, BuiltInFormulaReferences.TargetReturnsOutputs(extreme, new OmegaRatio(2, 0))["Or"][3]);
        Assert.Equal(Math.Sqrt(.5), BuiltInFormulaReferences.TargetReturnsOutputs(extreme, new UpsidePotentialRatio(2, 0))["Upr"][3]);
    }

    [Fact]
    public void NonrealOrNonfinitePeriodBenchmarksAreRejected()
    {
        foreach (var benchmark in new[] { double.NaN, double.PositiveInfinity, double.NegativeInfinity, -2d })
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => new OmegaRatioState(3, benchmark));
            Assert.Throws<ArgumentOutOfRangeException>(() => new UpsidePotentialRatioState(3, benchmark));
        }
    }
    ...
    public void InvalidFieldsNeverAdvanceAnyState()
    {
        ...
        {
            IStreamingIndicatorState Create() => variant == 0 ? new OmegaRatioState(3) : new UpsidePotentialRatioState(3);
    ...
    internal static readonly HashSet<string> Families = new(StringComparer.Ordinal) { "OmegaRatio", "UpsidePotentialRatio" };
