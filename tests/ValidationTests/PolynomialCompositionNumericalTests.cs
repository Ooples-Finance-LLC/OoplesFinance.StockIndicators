...
public sealed class PolynomialCompositionNumericalTests
{
    public static IEnumerable<object[]> Cases => IndicatorValidationDiscovery.Discover(new[] { typeof(IIndicator).Assembly })
        .Where(c => c.Name.StartsWith("polynomial-composition/", StringComparison.Ordinal))
    ...
    public void SharedDiscoveryIncludesEveryPromotedComposition()
    {
        ...
        foreach (var type in new[] { typeof(Tma), typeof(TriangularMovingAverage), typeof(SlowSmoothedMovingAverage) })
            Assert.Equal(9, cases.Count(c => c.IndicatorType == type));
    }
