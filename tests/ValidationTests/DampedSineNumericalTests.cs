...
public sealed class DampedSineNumericalTests
{
    ...
    public void CascadesPreserveStagesCancellationPreviewAndReset()
    {
        ...
        {
            var bars = BarsOf(prices); IBuiltInIndicator indicator = new DampedSineWaveWeightedFilter(length);
            var expected = BuiltInFormulaReferences.DampedSineOutputs(bars, length).Values.Single();
            var batch = Data(bars).CalculateDampedSineWaveWeightedFilter(length: length);
            Assert.Equal(expected, batch.CustomValuesList);
            var core = new double[bars.Length];
            OoplesFinance.StockIndicators.Core.MovingAverageCore.DampedSineWaveWeightedFilter(prices, core, length);
            ...
            using var actual = IndicatorCompute.ComputeArm(selected, new IndicatorSpec(indicator.BatchName, indicator.CreateOptions(), "Dswwf"), context);
            Assert.NotNull(actual); Assert.Equal(expected, actual.Value.ToArray());
            IStreamingIndicatorState state = new DampedSineWaveWeightedFilterState(length: length);
    ...
    public void FullConstantWindowsRetainUnitGainDespiteSignedWeights()
    {
        ...
        {
            var values = Data(BarsOf(Enumerable.Repeat(value, length + 5).ToArray())).CalculateDampedSineWaveWeightedFilter(length).CustomValuesList;
    ...
    public void InvalidFieldsNeverAdvanceAnyState()
    {
        ...
        {
            IStreamingIndicatorState state = new DampedSineWaveWeightedFilterState(length: 3);
            IStreamingIndicatorState control = new DampedSineWaveWeightedFilterState(length: 3);
    ...
    public static IEnumerable<object[]> Cases => IndicatorValidationDiscovery.Discover(new[] { typeof(IIndicator).Assembly })
        .Where(c => c.IndicatorType == typeof(DampedSineWaveWeightedFilter)).Select(c => new object[] { c });
    public static IEnumerable<object[]> Routes => Cases.SelectMany(c => new[] { "batch", "fast", "arm", "native", "streaming" }.Select(route => new object[] { c[0], route }));
    [Theory, MemberData(nameof(Routes))]
    public void AllRoutesMatchIndependentFormulasIncludingPreviewAndReset(IndicatorValidationCase testCase, string route) =>
        new OrdinalFamilyNumericalTests().CheckRoutes(testCase, route, bars => BuiltInFormulaReferences.DampedSineOutputs(bars, ((DampedSineWaveWeightedFilterSpecOptions)((IBuiltInIndicator)testCase.Factory()).CreateOptions()).Length), IndicatorErrorBudget.Exact);
