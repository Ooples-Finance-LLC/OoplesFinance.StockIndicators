...
public sealed class SineMeanNumericalTests
{
    [Fact]
    public void ConvexWeightsPreserveExtremesAndPreviewDoesNotCommit()
    {
        ...
        MovingAverageCore.SineWeightedMovingAverage(values, output, 1);
        Assert.Equal(values, output);
        MovingAverageCore.SineWeightedMovingAverage(Array.Empty<double>(), Span<double>.Empty, int.MaxValue);
        using var state = new OoplesFinance.StockIndicators.Helpers.SineWindowMean(3);
    ...
    public void NativeFactoriesHonorPeriodPreviewAndReset()
    {
        ...
        {
            var expected = BuiltInFormulaReferences.RoundedSineMean(fixture.Bars, length);
            var spec = new IndicatorSpec(IndicatorName.SineWeightedMovingAverage, new SineWmaSpecOptions(length));
            foreach (var state in new[] { StatefulIndicatorFactory.Create(spec), StreamingIndicatorFactory.CreateState(spec) })
            {
                ...
                for (var replay = 0; replay < 2; replay++)
                {
                    ...
                    for (var i = 0; i < fixture.Bars.Count; i++)
                    {
                        ...
                        Assert.Equal(expected[i], state.Update(bar, false, true).Outputs!["Swma"]);
    ...
    public static IEnumerable<object[]> Cases => IndicatorValidationDiscovery.Discover(new[] { typeof(IIndicator).Assembly })
        .Where(c => c.Factory() is IBuiltInIndicator b && b.BatchName == IndicatorName.SineWeightedMovingAverage)
    ...
    [InlineData(5)]
    [InlineData(65)]
    public async Task SineMeanMatchesAcrossRoutes(int period)
    {
        foreach (var fixture in IndicatorAdversarialCases.Generate(period == 14 ? 160 : 40, 250))
        {
            ...
            var expected = BuiltInFormulaReferences.RoundedSineMean(bars, period);
            var actual = new double[bars.Length];
            MovingAverageCore.SineWeightedMovingAverage(close, actual, period);
            Assert.Equal(expected, actual);
            var indicator = new SineWma(period);
            ...
            Assert.Equal(expected, data.CalculateSineWeightedMovingAverage(period).CustomValuesList);
            using var state = new SineWeightedMovingAverageState(period);
            for (var replay = 0; replay < 2; replay++)
            {
                state.Reset();
                for (var i = 0; i < bars.Length; i++)
                {
                    ...
                    foreach (var commit in new[] { false, true })
                    {
                        ...
                        Assert.Equal(expected[i], result.Outputs!["Swma"]);
                    }
                }
            }
        }
    ...
    public async Task SineMeanPreservesSelectedInput()
    {
        ...
        var expected = BuiltInFormulaReferences.RoundedSineMean(projected, 3);
        ...
        foreach (var chained in new[] { false, true })
        {
            ...
            using var result = IndicatorCompute.ComputeSineWmaFast(data, context, 3);
