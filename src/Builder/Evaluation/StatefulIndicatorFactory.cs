...
internal static partial class StatefulIndicatorFactory
{
    ...
    private static IStreamingIndicatorState? TryCreateFromTypedOptions(IndicatorSpec spec)
    {
        return spec.Options switch
        {
            ...
            TmaSpecOptions tma => new TriangularMovingAverageState(length: tma.Length),
            ...
            LsmaSpecOptions lsma => new LeastSquaresMovingAverageState(length: lsma.Length),
            ...
            T3SpecOptions t3 => new TillsonT3MovingAverageState(length: t3.Length),
            LinRegSpecOptions linreg => new LinearRegressionLineState(length: linreg.Length),

            // Oscillators - using named parameters where needed
            RsiSpecOptions rsi => new RelativeStrengthIndexState(maType: rsi.MaType, length: rsi.Length),
            ...
            TsiSpecOptions tsi => new TrueStrengthIndexState(length1: tsi.LongLength, length2: tsi.ShortLength),
            ...
            TrixSpecOptions trix => new TrixState(length: trix.Length),
            ...
            MacdLineSpecOptions macdl => new MovingAverageConvergenceDivergenceState(fastLength: macdl.FastLength, slowLength: macdl.SlowLength),
            MacdSignalSpecOptions macds => new MovingAverageConvergenceDivergenceState(fastLength: macds.FastLength, slowLength: macds.SlowLength, signalLength: macds.SignalLength),
            MacdHistogramSpecOptions macdh => new MovingAverageConvergenceDivergenceState(fastLength: macdh.FastLength, slowLength: macdh.SlowLength, signalLength: macdh.SignalLength),

            // Bollinger Bands
            BollingerBandsSpecOptions bb => new BollingerBandsState(bb.Length, bb.StdDevMult),

            // Volatility - using named parameters where MovingAvgType is first
            AtrSpecOptions atr => new AverageTrueRangeState(maType: atr.MaType, length: atr.Length),
            AdxSpecOptions adx => new AverageDirectionalIndexState(length: adx.Length),
            StdDevSpecOptions std => new StandardDeviationState(length: std.Length),
            HistoricalVolatilitySpecOptions hv => new HistoricalVolatilityState(length: hv.Length),
            ChaikinVolatilitySpecOptions cv => new ChaikinVolatilityState(length1: cv.Length),
            ...
            ForceIndexSpecOptions fi => new ForceIndexState(length: fi.Length),
            ...
            SuperTrendSpecOptions st => new SuperTrendState(length: st.Length),
            ...
            CoppockCurveSpecOptions cop => new CoppockCurveState(length: cop.Length),
            ...
            VmaSpecOptions vma => new VariableLengthMovingAverageState(minLength: vma.Length),
    ...
    private static IStreamingIndicatorState CreateMacdState(IndicatorOutput output, MacdSpecOptions macd)
    {
        ...
        return new MovingAverageConvergenceDivergenceState(fastLength: macd.FastLength, slowLength: macd.SlowLength, signalLength: macd.SignalLength);
    }
