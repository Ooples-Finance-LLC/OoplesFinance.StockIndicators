...
internal static partial class IndicatorCompute
{
    ...
    internal static ComputeBuffer? ComputeArm(StockData data, IndicatorSpec spec, ComputeContext context)
    {
        return spec.Options switch
        {
            ...
            MomentumSpecOptions mom => ComputeMomentumOscillatorFast(data, context, mom.Length),
            ...
            HammingMaSpecOptions hma2 => ComputeEhlersHammingMovingAverageFast(data, context, hma2.Length),
            ...
            RexOscillatorSpecOptions rexo => ComputeRexOscillatorFast(data, context, rexo.Length, rexo.MaType),
            ...
            SlowSmoothedMovingAverageSpecOptions ssma => ComputeSlowSmoothedMovingAverageFast(data, context, ssma.Length,
                ssma.MaType),
            RepulsionMovingAverageSpecOptions rema => ComputeRepulsionMovingAverageFast(data, context, rema.Length, rema.MaType),
            ...
            ElasticVolumeWeightedMovingAverageV1SpecOptions evwma => ComputeElasticVolumeWeightedMovingAverageV1Fast(data, context,
                evwma.Length, evwma.MaType),
            ...
            KurtosisIndicatorSpecOptions => ComputeKurtosisIndicatorFast(data, context),
            ...
            ErgodicMeanDeviationIndicatorSpecOptions emdi => ComputeErgodicMeanDeviationIndicatorFast(data, context, emdi.Length1, emdi.Length2,
                emdi.Length3, emdi.MaType),
    ...
    internal static ComputeBuffer ComputeGannHiLoActivatorFast(StockData data, ComputeContext context, int length = 3,
        MovingAvgType maType = MovingAvgType.SimpleMovingAverage)
    {
        // CalculateGannHiLoActivator flips between the averaged low and the averaged high as the chained
        // series crosses the previous bar's averages, and holds its last value in between.
        // TrendCore.GannHiLoActivator took the close and used the raw extremes rather than their averages.
        var inputList = data.ChainedValues.Count > 0 ? data.ChainedValues : data.InputValues;
        var input = SpanCompat.AsReadOnlySpan(inputList);
        var count = inputList.Count;
        length = Math.Max(length, 1);
        ...
        var buffer = context.Rent(count);
        var output = buffer.WritableSpan;
        var highMa = highAverage.Span;
        var lowMa = lowAverage.Span;
        for (var i = 0; i < count; i++)
        {
            var prevHighMa = i >= 1 ? highMa[i - 1] : 0;
            var prevLowMa = i >= 1 ? lowMa[i - 1] : 0;
            var previous = i >= 1 ? output[i - 1] : 0;

            output[i] = input[i] > prevHighMa ? lowMa[i] : input[i] < prevLowMa ? highMa[i] : previous;
        }
    ...
    /// </summary>
    internal static ComputeBuffer ComputeAutoLineFast(StockData data, ComputeContext context, int length = 500)
    {
        // CalculateAutoLine holds its line until the chained value escapes a band one windowed deviation either
        // side of it, then snaps the line to that value. TrendCore.AutoLine read the close and measured a
        // different band.
        var inputList = data.ChainedValues.Count > 0 ? data.ChainedValues : data.InputValues;
        var input = SpanCompat.AsReadOnlySpan(inputList);
        var count = inputList.Count;
        ...
        {
            var currentValue = input[i];
            var prevX = i >= 1 ? output[i - 1] : currentValue;

            output[i] = currentValue > prevX + dev[i] ? currentValue
                : currentValue < prevX - dev[i] ? currentValue
                : prevX;
        }
    ...
    {
        // CalculateGOscillator counts how many of the last length bars rose, expressed as a percentage of the
        // window. OscillatorCore.GOscillator read the high and the low, which is neither the chained series
        // nor the count.
        var inputList = data.ChainedValues.Count > 0 ? data.ChainedValues : data.InputValues;
        var count = inputList.Count;
        var input = SpanCompat.AsReadOnlySpan(inputList);
        ...
        {
            var previousValue = i >= 1 ? input[i - 1] : 0;
            window.Add(input[i] > previousValue ? (double)100 / length : 0);
    ...
    /// </summary>
    internal static ComputeBuffer ComputeGannTrendOscillatorFast(StockData data, ComputeContext context, int length = 3)
    {
        // CalculateGannTrendOscillator only ever publishes 1, -1 or its own previous value, decided by whether
        // the rolling extreme of the bar highs and lows has turned two bars running.
        // OscillatorCore.GannTrendOscillator scored a continuous series instead.
        var highs = SpanCompat.AsReadOnlySpan(data.HighPrices);
        var lows = SpanCompat.AsReadOnlySpan(data.LowPrices);
        var count = data.Count;
        length = Math.Max(length, 1);
        ...
        var highWindow = new RollingMinMax(length);
        var lowWindow = new RollingMinMax(length);
        double prevHighest1 = 0, prevHighest2 = 0, prevLowest1 = 0, prevLowest2 = 0;
        for (var i = 0; i < count; i++)
        {
            highWindow.Add(highs[i]);
            lowWindow.Add(lows[i]);
            ...
            var prevGto = i >= 1 ? output[i - 1] : 0;
            output[i] = prevHighest2 > prevHighest1 && highestHigh > prevHighest1 ? 1
                : prevLowest2 < prevLowest1 && lowestLow < prevLowest1 ? -1 : prevGto;

            prevHighest2 = i >= 1 ? prevHighest1 : 0;
            prevLowest2 = i >= 1 ? prevLowest1 : 0;
            prevHighest1 = highestHigh;
            prevLowest1 = lowestLow;
        }
    ...
    /// </summary>
    internal static ComputeBuffer ComputeTickLineMomentumOscillatorFast(StockData data, ComputeContext context, int length = 10,
        MovingAvgType maType = MovingAvgType.ExponentialMovingAverage, int smoothLength = 5)
    {
        // CalculateTickLineMomentumOscillator accumulates plus or minus one for each bar that closes above or
        // below the previous moving average, takes the rate of change of that running total over the smooth
        // window, and smooths it once more. The core routine this replaced skipped both later steps.
        var inputList = data.ChainedValues.Count > 0 ? data.ChainedValues : data.InputValues;
        var input = SpanCompat.AsReadOnlySpan(inputList);
        var count = inputList.Count;
        length = Math.Max(length, 1);
        ...
        var cumoSum = cumulative.WritableSpan;
        double running = 0;
        for (var i = 0; i < count; i++)
        {
            var prevMa = i >= 1 ? ma[i - 1] : 0;
            running += input[i] > prevMa ? 1 : input[i] < prevMa ? -1 : 0;
            cumoSum[i] = running;
        }
        ...
        for (var i = 0; i < count; i++)
        {
            var prevValue = i >= smoothLength ? cumoSum[i - smoothLength] : 0;
            roc[i] = prevValue != 0
                ? CalculationsHelper.MinPastValues(i, smoothLength, cumoSum[i] - prevValue) / prevValue * 100
                : 0;
        }
    ...
    /// </summary>
    internal static ComputeBuffer ComputeSlowSmoothedMovingAverageFast(StockData data, ComputeContext context, int length = 15,
        MovingAvgType maType = MovingAvgType.WeightedMovingAverage)
    {
        // CalculateSlowSmoothedMovingAverage splits the length into three sub-windows and runs the chained
        // series through an average of the given type once per window, each feeding the next. The core routine
        // this replaced took the close and offered no moving average type.
        var inputList = data.ChainedValues.Count > 0 ? data.ChainedValues : data.InputValues;
        var count = inputList.Count;

        var w2 = MathHelper.MinOrMax((int)Math.Ceiling(length / 3.0));
        var w1 = MathHelper.MinOrMax((int)Math.Ceiling((double)(length - w2) / 2));
        var w3 = MathHelper.MinOrMax((int)Math.Floor((double)(length - w2) / 2));

        using var first = context.Rent(count);
        using var second = context.Rent(count);
        MovingAverage(data, maType, w1, SpanCompat.AsReadOnlySpan(inputList), first.WritableSpan);
        MovingAverage(data, maType, w2, first.Span, second.WritableSpan);

        var buffer = context.Rent(count);
        MovingAverage(data, maType, w3, second.Span, buffer.WritableSpan);
    ...
    {
        // CalculateQuickMovingAverage weights the window with a triangle whose peak sits a third of the way
        // in, and it reaches one bar further back than the length. The core routine used a plain ramp.
        var inputList = data.ChainedValues.Count > 0 ? data.ChainedValues : data.InputValues;
        var input = SpanCompat.AsReadOnlySpan(inputList);
        var count = inputList.Count;
        length = Math.Max(length, 1);

        var peak = MathHelper.MinOrMax((int)Math.Ceiling((double)length / 3));
        ...
        int length = 40, MovingAvgType maType = MovingAvgType.SimpleMovingAverage, double mult = 20)
    {
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            output[i] = n > 0 ? (((n - currentVolume) * previous) + (currentVolume * currentValue)) / n : 0;
        }
    ...
    /// </summary>
    internal static ComputeBuffer ComputePentupleExponentialMovingAverageFast(StockData data, ComputeContext context, int length = 20,
        MovingAvgType maType = MovingAvgType.ExponentialMovingAverage)
    {
        // CalculatePentupleExponentialMovingAverage chains eight moving average passes over the chained
        // series and combines them by the binomial weights 8, -28, 56, -70, 56, -28, 8, -1. The core routine
        // this replaced read the close and used a shorter cascade.
        var inputList = data.ChainedValues.Count > 0 ? data.ChainedValues : data.InputValues;
        var count = inputList.Count;
        length = Math.Max(length, 1);
        ...
        using var seventh = context.Rent(count);
        using var eighth = context.Rent(count);
        ...
        MovingAverage(data, maType, length, sixth.Span, seventh.WritableSpan);
        MovingAverage(data, maType, length, seventh.Span, eighth.WritableSpan);
    ...
    /// </summary>
    internal static ComputeBuffer ComputeQuadrupleExponentialMovingAverageFast(StockData data, ComputeContext context, int length = 20,
        MovingAvgType maType = MovingAvgType.ExponentialMovingAverage)
    {
        // CalculateQuadrupleExponentialMovingAverage combines five successive averages with the binomial
        // coefficients that cancel four orders of lag. The core routine used a different combination.
        var inputList = data.ChainedValues.Count > 0 ? data.ChainedValues : data.InputValues;
        var count = inputList.Count;
        ...
        var ema1 = first.Span;
        var ema2 = second.Span;
        var ema3 = third.Span;
        var ema4 = fourth.Span;
        var ema5 = fifth.Span;
    ...
    {
        // CalculateSimpleLines projects the chained series forward by the last step of its own output, then
        // lets the line travel at most one step of 1/length towards that projection. The core routine this
        // replaced read the close and produced a different curve.
        var inputList = data.ChainedValues.Count > 0 ? data.ChainedValues : data.InputValues;
        var count = inputList.Count;
        var input = SpanCompat.AsReadOnlySpan(inputList);
        var step = 0.01 * 100 * (1.0 / Math.Max(1, length));
        ...
        {
            var currentValue = input[i];
            var previous1 = i >= 1 ? output[i - 1] : currentValue;
            var previous2 = i >= 2 ? output[i - 2] : currentValue;
            var projection = currentValue + ((previous1 - previous2) * mult);

            // The first bar has no line to travel from, so the projection itself becomes the reference and
            // the line starts there.
            var reference = i >= 1 ? output[i - 1] : projection;
            output[i] = projection > reference + step ? reference + step
                : projection < reference - step ? reference - step
                : reference;
        }
    ...
    {
        // CalculateHullEstimate leans a weighted average of the chained series against an exponential one of
        // the same half-length. The core routine this replaced produced a different curve from the same input.
        var inputList = data.ChainedValues.Count > 0 ? data.ChainedValues : data.InputValues;
        var count = inputList.Count;
        var input = SpanCompat.AsReadOnlySpan(inputList);
        var maLength = MathHelper.MinOrMax((int)Math.Ceiling(length / 2.0));

        using var weighted = context.Rent(count);
        using var exponential = context.Rent(count);
        MovingAverage(data, MovingAvgType.WeightedMovingAverage, maLength, input, weighted.WritableSpan);
        MovingAverage(data, MovingAvgType.ExponentialMovingAverage, maLength, input, exponential.WritableSpan);
    ...
    {
        // CalculateHybridConvolutionFilter blends its own previous output with each of the last length values
        // under a raised cosine weight, and the two cosine arguments are deliberately asymmetric: the first is
        // scaled by pi and the second is not. MovingAverageCore.HybridConvolutionFilter applied a symmetric
        // kernel, which is a different filter, so reproduce the batch term for term.
        var inputList = data.ChainedValues.Count > 0 ? data.ChainedValues : data.InputValues;
        var input = SpanCompat.AsReadOnlySpan(inputList);
        var count = inputList.Count;
        length = Math.Max(length, 1);
    ...
    /// </summary>
    internal static ComputeBuffer ComputeQuadraticMovingAverageFast(StockData data, ComputeContext context, int length = 14)
    {
        // CalculateQuadraticMovingAverage is the root mean square of the chained series over the window, and
        // it averages the partial window from the first bar rather than waiting for a full one.
        var inputList = data.ChainedValues.Count > 0 ? data.ChainedValues : data.InputValues;
        var count = inputList.Count;
        var input = SpanCompat.AsReadOnlySpan(inputList);
        length = Math.Max(1, length);
        ...
        for (var i = 0; i < count; i++)
        {
            window.Add(MathHelper.Pow(input[i], 2));

            var meanSquare = window.Average(length);
            output[i] = meanSquare >= 0 ? MathHelper.Sqrt(meanSquare) : 0;
        }
    ...
    internal static ComputeBuffer ComputeSequentiallyFilteredMovingAverageFast(StockData data, ComputeContext context, int length = 50,
        MovingAvgType maType = MovingAvgType.SimpleMovingAverage)
    {
        // CalculateSequentiallyFilteredMovingAverage only lets its output move to the moving average on the
        // bars where that average has been going the same way for the whole window; on every other bar it
        // holds. MovingAverageCore.SequentiallyFilteredMovingAverage filtered on something else and never saw
        // the requested average type at all.
        var inputList = data.ChainedValues.Count > 0 ? data.ChainedValues : data.InputValues;
        var count = inputList.Count;
        length = Math.Max(length, 1);
        ...
        var signSum = new RollingSum();
        for (var i = 0; i < count; i++)
        {
            var prevSma = i >= 1 ? sma[i - 1] : 0;
    ...
    {
        // CalculateTFSTetherLineIndicator is the midpoint of the window's extremes, taken over the range that
        // belongs to the chained series rather than the bar's own high and low. The core routine this replaced
        // always used the price extremes.
        var inputList = data.ChainedValues.Count > 0 ? data.ChainedValues : data.InputValues;
        var count = inputList.Count;

        using var highRange = context.Rent(count);
        using var lowRange = context.Rent(count);
        CustomRange(data, SpanCompat.AsReadOnlySpan(inputList), highRange.WritableSpan, lowRange.WritableSpan);
        var highs = highRange.Span;
        var lows = lowRange.Span;

        var buffer = context.Rent(count);
        var output = buffer.WritableSpan;
        var highWindow = new RollingMinMax(Math.Max(1, length));
        var lowWindow = new RollingMinMax(Math.Max(1, length));
        for (var i = 0; i < count; i++)
        {
            highWindow.Add(highs[i]);
            lowWindow.Add(lows[i]);
    ...
    /// </summary>
    internal static ComputeBuffer ComputeMomentumOscillatorFast(StockData data, ComputeContext context, int length = 14)
    {
        // CalculateMomentumOscillator publishes the raw ratio of the chained series to its value length bars
        // back as its primary series - the moving average of it is the separate Signal key - so no smoothing
        // belongs here. That is also why the spec's SmoothLength is marked as having no effect, and why MaType
        // cannot reach this series.
        var inputList = data.ChainedValues.Count > 0 ? data.ChainedValues : data.InputValues;
        var input = SpanCompat.AsReadOnlySpan(inputList);
        var count = inputList.Count;
        length = Math.Max(length, 1);
        ...
        {
            var priorValue = i >= length ? input[i - length] : 0;
            output[i] = priorValue != 0 ? input[i] / priorValue * 100 : 0;
        }
    ...
    /// </summary>
    internal static ComputeBuffer ComputeDema2LinesFast(StockData data, ComputeContext context, int fastLength = 10,
        int slowLength = 40, MovingAvgType maType = MovingAvgType.ExponentialMovingAverage)
    {
        // CalculateDema2Lines publishes its fast line as the chained series smoothed twice over the fast
        // length - not the usual double exponential average, which leans the first pass against the second.
        // The switch this replaced computed that other average and fell back to it for most types anyway.
        var inputList = data.ChainedValues.Count > 0 ? data.ChainedValues : data.InputValues;
        var count = inputList.Count;

        using var firstPass = context.Rent(count);
        MovingAverage(data, maType, fastLength, SpanCompat.AsReadOnlySpan(inputList), firstPass.WritableSpan);

        var buffer = context.Rent(count);
        MovingAverage(data, maType, fastLength, firstPass.Span, buffer.WritableSpan);
        ...
        int length2 = 5, int length3 = 5, MovingAvgType maType = MovingAvgType.ExponentialMovingAverage)
    {
        ...
        using var smoothed = context.Rent(count);
        MovingAverage(data, maType, length2, deviation.Span, smoothed.WritableSpan);
    ...
    /// </summary>
    internal static ComputeBuffer ComputeJapaneseCorrelationCoefficientFast(StockData data, ComputeContext context, int length = 50,
        MovingAvgType maType = MovingAvgType.SimpleMovingAverage)
    {
        // CalculateJapaneseCorrelationCoefficient divides how far the smoothed chained series has travelled
        // over the full length by the range of the smoothed highs and lows. Every smoothing, and both windows,
        // run at half the length, which is the only place the length appears other than that lookback.
        var inputList = data.ChainedValues.Count > 0 ? data.ChainedValues : data.InputValues;
        var count = inputList.Count;
        length = Math.Max(length, 1);
        ...
        using var smoothedLow = context.Rent(count);
        using var smoothedInput = context.Rent(count);
        MovingAverage(data, maType, length1, SpanCompat.AsReadOnlySpan(data.HighPrices), smoothedHigh.WritableSpan);
        MovingAverage(data, maType, length1, SpanCompat.AsReadOnlySpan(data.LowPrices), smoothedLow.WritableSpan);
        MovingAverage(data, maType, length1, SpanCompat.AsReadOnlySpan(inputList), smoothedInput.WritableSpan);
        var h = smoothedHigh.Span;
        var l = smoothedLow.Span;
        var c = smoothedInput.Span;
    ...
    /// </summary>
    internal static ComputeBuffer ComputeNaturalDirectionalComboFast(StockData data, ComputeContext context, int length = 40,
        int smoothLength = 20, MovingAvgType maType = MovingAvgType.ExponentialMovingAverage)
    {
        // CalculateNaturalDirectionalCombo multiplies the natural directional index by the natural stochastic
        // indicator, both read off the caller's own series with the same two lengths, and takes the signed
        // square root of that. Where the two agree on direction the product is replaced by the average of each
        // one scaled by the other's magnitude, which keeps the sign the pair already share.
        var count = data.Count;

        using var directional = ComputeNaturalDirectionalIndexFast(data, context, length, smoothLength, maType);
        using var stochastic = ComputeNaturalStochasticIndicatorFast(data, context, length, smoothLength, maType);
        var ndxSpan = directional.Span;
        var nstSpan = stochastic.Span;
    ...
    /// </summary>
    internal static ComputeBuffer ComputeNaturalDirectionalIndexFast(StockData data, ComputeContext context, int length = 40,
        int smoothLength = 20, MovingAvgType maType = MovingAvgType.ExponentialMovingAverage)
    {
        // CalculateNaturalDirectionalIndex works on the natural logarithm of the chained series, weighting
        // each lookback by the inverse square root of its distance and dividing by a running absolute sum
        // that grows across the window. The core routine implemented none of that.
        var inputList = data.ChainedValues.Count > 0 ? data.ChainedValues : data.InputValues;
        var input = SpanCompat.AsReadOnlySpan(inputList);
        var count = inputList.Count;
        length = Math.Max(length, 1);
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            {
                var prevLn = i >= j + 1 ? ln[i - (j + 1)] : 0;
                var currLn = i >= j ? ln[i - j] : 0;
                absSum += Math.Abs(prevLn - currLn);
                var frac = absSum != 0 ? (ln[i] - currLn) / absSum : 0;
                var ratio = 1 / MathHelper.Sqrt(j + 1);
                weightSum += frac * ratio;
                denomSum += ratio;
            }

            rawNdx[i] = denomSum != 0 ? weightSum / denomSum * 100 : 0;
        }

        var buffer = context.Rent(count);
        MovingAverage(data, maType, smoothLength, raw.Span, buffer.WritableSpan);
        return buffer;
    }
    ...
    /// </summary>
    internal static ComputeBuffer ComputeNaturalMarketMirrorFast(StockData data, ComputeContext context, int length = 40,
        MovingAvgType maType = MovingAvgType.ExponentialMovingAverage)
    {
        // CalculateNaturalMarketMirror publishes "Nmm": the moving average of the mean square root weighted
        // change in the log of the chained series across the whole window. The two case switch this replaced was
        // a plain moving average of the close.
        var inputList = data.ChainedValues.Count > 0 ? data.ChainedValues : data.InputValues;
        var input = SpanCompat.AsReadOnlySpan(inputList);
        var count = inputList.Count;
        length = Math.Max(length, 1);
        ...
        var oiAvg = weightedAverage.WritableSpan;
        for (var i = 0; i < count; i++)
        {
            double sum = 0;
            for (var j = 1; j <= length; j++)
            {
                var prevLn = i >= j ? ln[i - j] : 0;

                sum += (ln[i] - prevLn) / MathHelper.Sqrt(j) * 100;
            }
        ...
        var buffer = context.Rent(count);
        MovingAverage(data, maType, length, weightedAverage.Span, buffer.WritableSpan);
        return buffer;
    }
    ...
    /// </summary>
    internal static ComputeBuffer ComputeNaturalMarketRiverFast(StockData data, ComputeContext context, int length = 40,
        MovingAvgType maType = MovingAvgType.ExponentialMovingAverage)
    {
        // CalculateNaturalMarketRiver publishes "Nmr": the moving average of a square root weighted sum of the
        // bar to bar changes in the log of the chained series. The two case switch this replaced was a plain
        ...
        using var weightedSum = context.Rent(count);
        var oiSum = weightedSum.WritableSpan;
        for (var i = 0; i < count; i++)
        {
            double sum = 0;
            for (var j = 0; j < length; j++)
            {
                ...
                sum += (prevLn - currentLn) * (MathHelper.Sqrt(j) - MathHelper.Sqrt(j + 1));
            }
        ...
        var buffer = context.Rent(count);
        MovingAverage(data, maType, length, weightedSum.Span, buffer.WritableSpan);
        return buffer;
    }
    ...
    /// </summary>
    internal static ComputeBuffer ComputeNaturalStochasticIndicatorFast(StockData data, ComputeContext context, int length = 20,
        int smoothLength = 10, MovingAvgType maType = MovingAvgType.ExponentialMovingAverage)
    {
        // CalculateNaturalStochasticIndicator averages the stochastic position of every bar in the window
        // against the window's own high and low AS THEY STOOD at that bar, each weighted by the inverse
        // square root of its distance, then smooths the result. The core routine took a single position.
        var inputList = data.ChainedValues.Count > 0 ? data.ChainedValues : data.InputValues;
        var input = SpanCompat.AsReadOnlySpan(inputList);
        var highs = SpanCompat.AsReadOnlySpan(data.HighPrices);
        var lows = SpanCompat.AsReadOnlySpan(data.LowPrices);
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            rawNst[i] = denomSum != 0 ? (200 * weightSum / denomSum) - 100 : 0;
        }
        ...
        int length2 = 10, MovingAvgType maType = MovingAvgType.ExponentialMovingAverage)
    {
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            running = currentValue > previousValue ? running + volume
                : currentValue < previousValue ? running - volume
                : running;
            obv[i] = running;
        }
