...
internal static partial class IndicatorCompute
{
    private static ComputeBuffer ComputeLogisticCorrelation(StockData data, ComputeContext context, int length, double k)
    {
        ...
        using var correlation = new OoplesFinance.StockIndicators.Streaming.RollingWindowCorrelation(length);
        for (var i = 0; i < values.Count; i++)
            result.WritableSpan[i] = 1 / (1 + Math.Exp(Math.Min(100, -k * correlation.Add(i, values[i], out _))));
        return result;
    }
