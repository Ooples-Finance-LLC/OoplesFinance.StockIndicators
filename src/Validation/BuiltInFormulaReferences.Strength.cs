...
internal static partial class BuiltInFormulaReferences
{
    ...
    private static ReferenceFraction[] SmoothStrengthStage(ReferenceFraction[] input, int length, int kind)
    {
    ...
    internal static IReadOnlyDictionary<string, double[]> StrengthOutputs(IReadOnlyList<Bar> bars, IBuiltInIndicator indicator)
    {
        ...
        double[] Strength(int[] periods)
        {
            ...
            foreach (var period in periods) { numerator = SmoothStrengthStage(numerator, period, kind); denominator = SmoothStrengthStage(denominator, period, kind); }
