...
{
    internal static double[] RoundedHannMean(IReadOnlyList<Bar> bars, int length)
    {
        var coefficients = Enumerable.Range(1, length)
            .Select(j => ReferenceFraction.FromDouble(1 - Math.Cos(2 * Math.PI * (j / (length + 1d))))).ToArray();
