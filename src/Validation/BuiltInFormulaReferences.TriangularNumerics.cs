...
internal static partial class BuiltInFormulaReferences
{
    ...
    internal static bool HasBoundedTriangularMean(IBuiltInIndicator indicator) =>
        indicator.BatchName == IndicatorName.TriangularMovingAverage && BoundedMeanKind(indicator.CreateOptions(), 1) is 1 or 2 or 3 or 6 or 7 or 8 or 9 or 10 or 11 or 12 or 13 or 14 or 15 or 16 or 17 or 18 or 19;

    private static double[] RoundedBoundedStage(IReadOnlyList<double> values, int length, int kind)
    {
        if (kind is 8 or 9 or 10 or 11 or 12 or 13 or 14 or 15 or 16 or 17 or 18 or 19)
        {
            var bars = values.Select(value => new Bar(default, value, value, value, value, 0)).ToArray();
            return kind == 19 ? RoundedVidya(bars, length) : kind == 18 ? RoundedHannMean(bars, length) : kind == 16 ? RoundedSineMean(bars, length) : kind == 17 ? RoundedNaturalMean(bars, length)
