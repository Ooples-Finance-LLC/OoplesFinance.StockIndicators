...
internal static partial class BuiltInFormulaReferences
{
    private static int BoundedMeanKind(object options, int fallback)
    {
        ...
        if (value is MovingAvgType.EhlersHammingMovingAverage) return 21;
    ...
    internal static bool HasBoundedTriangularMean(IBuiltInIndicator indicator) =>
        indicator.BatchName == IndicatorName.TriangularMovingAverage && BoundedMeanKind(indicator.CreateOptions(), 1) is 1 or 2 or 3 or 6 or 7 or 8 or 9 or 10 or 11 or 12 or 13 or 14 or 15 or 16 or 17 or 18 or 19 or 20 or 21;

    private static double[] RoundedBoundedStage(IReadOnlyList<double> values, int length, int kind)
    {
        if (kind is 8 or 9 or 10 or 11 or 12 or 13 or 14 or 15 or 16 or 17 or 18 or 19 or 20 or 21)
        {
            var bars = values.Select(value => new Bar(default, value, value, value, value, 0)).ToArray();
            return kind == 21 ? RoundedHammingMean(bars, length) : kind == 20 ? RoundedAlmaMean(bars, length) : kind == 19 ? RoundedVidya(bars, length) : kind == 18 ? RoundedHannMean(bars, length) : kind == 16 ? RoundedSineMean(bars, length) : kind == 17 ? RoundedNaturalMean(bars, length)
