...
public class DispersionConsumerGenerator : IIncrementalGenerator
{
    ...
    private static void Emit(SourceProductionContext context, ImmutableArray<ImmutableArray<DispersionUse>?> items)
    {
        ...
        sb.AppendLine("/// Every indicator taking its dispersion from CalculateStandardDeviationVolatility, and the");
        sb.AppendLine("/// series each asks for it on. Read from the calculations, so it cannot drift from them.");
        sb.AppendLine("/// </summary>");
        sb.AppendLine("public static class GeneratedDispersionConsumers");
        sb.AppendLine("{");
