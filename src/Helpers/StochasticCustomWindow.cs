...
internal sealed class StochasticCustomWindow : IDisposable
{
    ...
    internal StochasticCustomWindow(MovingAvgType kind, int rangeLength, int firstLength, int signalLength)
    {
        _high = new(Math.Max(1, rangeLength)); _low = new(Math.Max(1, rangeLength));
        var lengths = new[] { firstLength, firstLength, signalLength };
    ...
    {
        var distance = new ExactMeanAccumulator(); distance.Add(price); distance.Add(low, -1);
    ...
    internal static double Ratio(RocBankValue distance, RocBankValue range)
    {
        ...
        return MathHelper.MinOrMax(numerator.Ratio(denominator), 100, 0);
    }
    ...
    internal (double Line, double Signal) Next(double price, double high, double low, bool commit)
    {
        ...
        var distance = Smooth(0, components.Distance, commit);
        var range = Smooth(1, components.Range, commit);
        var line = Ratio(distance, range);
        return (line, Smooth(2, new(line), commit).Publish());
    }
    internal void Reset()
    {
