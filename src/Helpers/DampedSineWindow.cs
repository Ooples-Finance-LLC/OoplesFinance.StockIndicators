...
internal sealed class DampedSineWindow : IDisposable
{
    private readonly double[] _weights;
    private readonly ExactMeanAccumulator _denominator;
    private readonly PooledRingBuffer<double> _values;

    internal DampedSineWindow(int length)
    {
        ...
        {
            _weights[lag] = Math.Sin(2 * Math.PI * (lag + 1d) / length) / (lag + 1d);
    ...
    internal static void Compute(ReadOnlySpan<double> input, Span<double> output, int length)
    {
        ...
        using var mean = new DampedSineWindow(length);
