...
The CI test `NumericalEnrollmentCoverageTests` compares the validator's shared numerical-fixture policy with the exact configuration names in `tests/ValidationTests/NumericalFixtureBacklog.txt`. New omissions and enrollment regressions fail CI. Completed configurations also fail until their obsolete backlog entries are removed. This migration backlog records unfinished work; it does not exempt any configuration from the publication gate, which requires completed numerical evidence for every discovered configuration. Customer indicators and built-ins that consume them remain automatically enrolled. Do not regenerate the backlog to admit new omissions.
