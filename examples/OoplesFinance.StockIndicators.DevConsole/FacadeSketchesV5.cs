...
internal static class FacadeSketchesV5
{
    ...
    {
        Console.WriteLine();
        Console.WriteLine("Facade/builder sketches (v5):");
        RunOptionA();
        RunOptionB();
        RunOptionC();
    }
    ...
    {
        Console.WriteLine();
        Console.WriteLine("Option A: Unified flow (batch source)");
        ...
        ConfigureCorePipeline(builder);
        var runtime = builder.Build();
        runtime.Start();

        if (runtime.Latest != null)
        {
            PrintLast(runtime.Latest, IndicatorKey.Sma, "SMA(20)");
            PrintLast(runtime.Latest, IndicatorKey.Rsi, "RSI(14)");
            PrintLast(runtime.Latest, IndicatorKey.Macd, "MACD");
        }
    }
    ...
    {
        Console.WriteLine();
        Console.WriteLine("Option B: Unified flow (streaming source, auto options)");

        var trades = BuildTradeEvents(Symbol, 12);
        var stream = new ReplayStreamSource(trades);
        var source = IndicatorDataSource.FromStreaming(stream, Symbol);
    ...
    {
        Console.WriteLine();
        Console.WriteLine("Option C: Same config for batch and streaming");

        var batchData = BuildSampleData(180, 99d, new DateTime(2024, 11, 2, 9, 30, 0, DateTimeKind.Utc));
        var batchBuilder = new StockIndicatorBuilder(IndicatorDataSource.FromBatch(new StockData(batchData)));
        var batchHandles = ConfigureAdvancedPipeline(batchBuilder);
        var batchRuntime = batchBuilder.Build();
        batchRuntime.Start();

        if (batchRuntime.Latest != null)
        {
            PrintLast(batchRuntime.Latest, batchHandles.Rsi, "RSI(14)");
            PrintLast(batchRuntime.Latest, batchHandles.RsiSma, "RSI SMA(5)");
            PrintLast(batchRuntime.Latest, batchHandles.Spread, "SMA - RSI");
        }

        var trades = BuildTradeEvents(Symbol, 12);
        var stream = new ReplayStreamSource(trades);
        var streamBuilder = new StockIndicatorBuilder(IndicatorDataSource.FromStreaming(stream, Symbol));
        var streamHandles = ConfigureAdvancedPipeline(streamBuilder);
        var streamRuntime = streamBuilder.Build();
        streamRuntime.Start();

        if (streamRuntime.Latest != null)
        {
            PrintLast(streamRuntime.Latest, streamHandles.Rsi, "RSI(14)");
            PrintLast(streamRuntime.Latest, streamHandles.RsiSma, "RSI SMA(5)");
            PrintLast(streamRuntime.Latest, streamHandles.Spread, "SMA - RSI");
        }
    }

    private static void ConfigureCorePipeline(StockIndicatorBuilder builder)
    {
        builder
    ...
    private static PipelineHandles ConfigureAdvancedPipeline(StockIndicatorBuilder builder)
    {
        ...
        builder
            ...
            {
                var price = ind.Price();
                var sma = ind.Sma(20, price, IndicatorKey.Sma);
                rsi = ind.Rsi(14, price, IndicatorKey.Rsi);
        ...
        return new PipelineHandles(rsi, rsiSma, spread);
    }

    private readonly struct PipelineHandles
    {
        public PipelineHandles(SeriesHandle rsi, SeriesHandle rsiSma, SeriesHandle spread)
        {
            Rsi = rsi;
            RsiSma = rsiSma;
            Spread = spread;
        ...
        public SeriesHandle Rsi { get; }
        public SeriesHandle RsiSma { get; }
        public SeriesHandle Spread { get; }
    }
    ...
    private sealed class IndicatorDataSource
    {
        ...
        public static IndicatorDataSource FromStreaming(IStreamSource source, string symbol, BarTimeframe timeframe)
        {
            var options = CreateDefaultStreamingOptions(symbol);
            return new IndicatorDataSource(IndicatorSourceKind.Streaming, null, source, options, timeframe);
        }

        public static IndicatorDataSource FromStreaming(IStreamSource source, string symbol)
        {
            return FromStreaming(source, symbol, BarTimeframe.Tick);
        }
        ...
        private static StreamingOptions CreateDefaultStreamingOptions(string symbol)
        {
